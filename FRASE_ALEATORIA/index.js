...
button1.addEventListener('click', function(){
    cita.classList.add('fading-out');
    setTimeout(() => {
        cita.innerHTML = mostrarCita(citas);
        cita.classList.remove('fading-out');
    }, 500);
});

// Añadimos un event listener para el botón
toggleButton.addEventListener("click", function() {
    ...
    if(body.classList.contains("dark-mode")) {
        toggleButton.textContent = "Modo Claro"; // Si está en modo nocturno, cambiar el texto del botón
    } else {
        toggleButton.textContent = "Modo Nocturno"; // Si está en modo claro, cambiar el texto del botón
    }
});
